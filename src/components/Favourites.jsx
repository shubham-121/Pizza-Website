...
export default function Favourites() {
  ...
  function handleSortType(e) {
    ...
    if (sortOption === "price") {
      sortedFavItems = [...favourites].sort((a, b) => b.price - a.price);
      dispatch(sortFavourites(sortedFavItems));
    } else if (sortOption === "quantity") {
      sortedFavItems = [...favourites].sort((a, b) => b.quantity - a.quantity);
      dispatch(sortFavourites(sortedFavItems));
    } else if (sortOption === "name") {
      sortedFavItems = [...favourites].sort((a, b) =>
    ...
    <div>
      ...
      <div className="overflow-y-scroll min-h-[90vh] max-w-[100vw] mt-6 ml-5 mr-5 border-solid border-2 border-black">
        ...
        <p className="text-center italic text-xl font-semibold underline border-2 mt-4 ml-4 mr-4">
          {totalFavItems
            ? `Your '${totalFavItems}' Favourite  Pizzas!😝`
            : "Oops No Pizzas In Your Favourite List For Now!😥"}
        </p>
        <div>
          {favourites.map((item, idx) => (
            <RenderFavourites item={item} key={idx}></RenderFavourites>
          ))}
        </div>
      </div>
...
function RenderFavourites({ item }) {
  const dispatch = useDispatch();
